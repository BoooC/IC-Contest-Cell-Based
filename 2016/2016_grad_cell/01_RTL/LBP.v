...
reg [2:0] init_count, next_init_count;
reg [6:0] addr_x, addr_y;
reg [6:0] next_addr_x, next_addr_y;

reg [7:0] gray_buffer [0:7];
...
always @(*) begin
	if(state == INIT) begin
		next_init_count = init_done ? 'd0 : (init_count + 'd1);
	end
	else if(state == EXE) begin
		next_init_count = exe_done ? 'd0 : (init_count + 'd1);
	end
...
always @(*) begin
	if(exe_done) begin
		next_addr_x = addr_x_done ? 'd1 : (addr_x + 'd1);
	end
	else if(state == DONE) begin
		next_addr_x = 'd1;
	end
...
always @(*) begin
	if(row_done) begin
		next_addr_y = addr_y_done ? 'd1 : (addr_y + 'd1);
	end
	else if(state == DONE) begin
		next_addr_y = 'd1;
	end
...
always @(posedge clk) begin
	if (next_state == INIT) begin
		case(next_init_count)
			'd0 : gray_addr <= {next_addr_y - 7'd1	, next_addr_x - 7'd1};
			'd1 : gray_addr <= {next_addr_y		  	, next_addr_x - 7'd1};
			'd2 : gray_addr <= {next_addr_y + 7'd1	, next_addr_x - 7'd1};
			'd3 : gray_addr <= {next_addr_y - 7'd1	, next_addr_x};
			'd4 : gray_addr <= {next_addr_y			, next_addr_x};
			'd5 : gray_addr <= {next_addr_y + 7'd1	, next_addr_x};
