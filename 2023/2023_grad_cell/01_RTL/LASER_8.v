...
reg [3:0] opt_C2Y;
reg [5:0] opt_count;
reg [5:0] inside_count;

reg c_update_flag;
reg [2:0] unchange_count;
...
wire [5:0] point_count = count << 3;

wire [3:0] point_x [0:7];
wire [3:0] point_y [0:7];
assign point_x[0] = X_reg[point_count+0];
assign point_x[1] = X_reg[point_count+1];
assign point_x[2] = X_reg[point_count+2];
assign point_x[3] = X_reg[point_count+3];
assign point_x[4] = X_reg[point_count+4];
assign point_x[5] = X_reg[point_count+5];
assign point_x[6] = X_reg[point_count+6];
assign point_x[7] = X_reg[point_count+7];
assign point_y[0] = Y_reg[point_count+0];
assign point_y[1] = Y_reg[point_count+1];
assign point_y[2] = Y_reg[point_count+2];
assign point_y[3] = Y_reg[point_count+3];
assign point_y[4] = Y_reg[point_count+4];
assign point_y[5] = Y_reg[point_count+5];
assign point_y[6] = Y_reg[point_count+6];
assign point_y[7] = Y_reg[point_count+7];
...
wire [5:0] next_indise_count =	inside_count + is_inside[0] + is_inside[1] + is_inside[2] + is_inside[3] + 
								is_inside[4] + is_inside[5] + is_inside[6] + is_inside[7];
wire update_flag = next_indise_count >= opt_count;

wire current_cx_done = current_cx == 'd15;
wire current_cy_done = current_cy == 'd15;

// control
wire din_done = count == 'd39;
wire current_c_done = count == 'd4;
wire find_c_done = current_c_done & current_cx_done & current_cy_done;
wire converge = unchange_count == 'd3;
...
always @(posedge CLK) begin
	if (RST) begin
		...
		current_cx <= current_cx_done ? 'd0 : (current_cx + 'd1);
	end
end
...
always @(posedge CLK) begin
	if (RST) begin
		...
		current_cy <= current_cy_done ? 'd0 : (current_cy + 'd1);
	end
end
...
always @(posedge CLK) begin
	if (RST) begin
		...
		count <= current_c_done ? 'd0 : (count + 'd1);
	end
end
...
always @(posedge CLK) begin
	if (RST) begin
		...
		inside_count <= next_indise_count;
	end
end
...
always @(posedge CLK) begin
	...
	else if (current_c_done & next_indise_count > opt_count) begin
		c_update_flag <= 1'b1;
	end
end
...
wire signed [4:0] diff_x = C_X - X;
wire signed [4:0] diff_y = C_Y - Y;

wire [3:0] dis_x = diff_x[4] ? (~diff_x + 'd1) : diff_x;
wire [3:0] dis_y = diff_y[4] ? (~diff_y + 'd1) : diff_y;

assign is_inside = 	(dis_x == 'd4 & dis_y == 'd0) | (dis_x == 'd0 & dis_y == 'd4) | 
					(dis_x == 'd3 & dis_y <= 'd2) | (dis_y == 'd3 & dis_x <= 'd2) | 
					(dis_x == 'd2 & dis_y <= 'd3) | (dis_y == 'd2 & dis_x <= 'd3) | 
					(dis_x == 'd1 & dis_y <= 'd3) | (dis_y == 'd1 & dis_x <= 'd3) | 
					(dis_x == 'd0 & dis_y <= 'd4) | (dis_y == 'd0 & dis_x <= 'd4);
